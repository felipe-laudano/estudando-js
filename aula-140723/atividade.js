...
function somaVetor(callback) {
    let total = callback([2, 4, 6, 8, 10]);
    console.log(total);
}
somaVetor(num => num.reduce((acumulador, num) => (acumulador + num)));

//Crie uma função que receba um vetor de palavras e retorne a concatenação de todas as palavras em uma única string.
function concString(callback) {
    let concatenadas = callback([`Banana`, `Maca`, `Pera`, `Uva`])
    console.log(concatenadas);
}
concString(palavras => palavras.join(""));
